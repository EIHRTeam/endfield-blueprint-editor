...
class BlueprintBaker:
    ...
    def active_connection_frame(self, size):
        ...
        def sample(path, attribute):
            ...
            keys = curve['curve']['m_Curve']; time = animation['sampleTime']
            if time <= keys[0]['time']: return keys[0]['value']
            for a, b in zip(keys, keys[1:]):
                if time <= b['time']:
                    duration = b['time'] - a['time']; t = (time - a['time']) / duration
        ...
        for path in ['SelectedFrame1', 'SelectedFrame2']:
            ...
            center(out, self.scaled(key, inner_size, use_slice=True, rgba=(255, 255, 255, round(sample(path, 'm_Color.a') * 255))))
        return out
    ...
    def bake_logistics(self, destination=None):
        destination = destination or self.kit / 'dist/baked/logistics'
        manifest = {}
        for bid, entry in self.logistics.items():
            folder = destination / bid
            folder.mkdir(parents=True, exist_ok=True)
            for direction in range(4):
                self.body(bid, direction).save(folder / f'd{direction}.png')
            manifest[bid] = {'itemId': entry['itemId'], 'size': [UNIT, UNIT],
                             'faces': [f'{bid}/d{d}.png' for d in range(4)]}
    ...
    def bake_all(self, destination):
        destination.mkdir(parents=True, exist_ok=True)
        manifest = {'pixelsPerCell': UNIT, 'direction': '0=native, 1/2/3=90/180/270 degrees clockwise; symbols stay upright',
                    'position': 'PNG top-left placement; original local +z points toward PNG top',
                    'formulaMode': None, 'buildings': {}}
        for bid in self.buildings:
            directory = destination / bid
            directory.mkdir(exist_ok=True)
            entry = {'width': self.buildings[bid]['range']['width'], 'depth': self.buildings[bid]['range']['depth'], 'faces': []}
            self.icon(bid).save(directory / 'icon.png')
            for face in range(4):
                body = self.body(bid, face)
                body.save(directory / f'd{face}_body.png')
                with_icon = body.copy()
                center(with_icon, self.icon(bid))
                with_icon.save(directory / f'd{face}.png')
                entry['faces'].append({'body': f'{bid}/d{face}_body.png', 'image': f'{bid}/d{face}.png', 'size': list(body.size)})
            manifest['buildings'][bid] = entry
        manifest['limitations'] = sorted(self.issues)
        (destination / 'index.json').write_text(json.dumps(manifest, indent=2), encoding='utf-8')
        return manifest
