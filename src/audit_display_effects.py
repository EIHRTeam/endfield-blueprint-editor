...
def audit_effects(root):
    ...
    report = {'scope': 'All 85 image components recovered from BlueprintPreview; static export and editor interaction are distinguished.',
              'source': 'assets/blueprint_source/prefab_images.json', 'layers': len(rows),
              'counts': dict(Counter(row['status'] for row in rows)), 'rows': rows,
              'rightPanelReview': {
                  'sources': ['docs/project-notes.md (BlueprintContent provenance)', 'assets/blueprint_presentation/prefab_components.json'],
                  'implemented': ['cover and paper', 'name/creator/size/tags/description', 'inventory grouping and sorting', 'item artwork, rarity and count', 'native frame and gradient artwork'],
                  'approximated': ['card shadow and count text shadow', 'tag capsules and empty-tag outline', 'name brackets and ellipsis', 'scrollbar decoration'],
                  'notReproduced': ['interactive card selection/flash animations', 'game inventory shortage, technology, unlock, expiry warning panels', 'controller hints and share/review actions']}}
    (root / 'reports/effect-layer-audit.json').write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding='utf-8')
    if report['counts'].get('unreviewed'):
        raise ValueError('Unreviewed preview layers; see reports/effect-layer-audit.json')
    return report
