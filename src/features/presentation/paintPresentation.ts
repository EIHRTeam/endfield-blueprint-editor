...
function paintContext(scene: SceneValue, buildings: Parameters<typeof paintScene>[2]['buildings']) {
  return {
    assets: scene.assets,
    buildings,
...
function buildingIndex(scene: SceneValue) {
 ...
 */
export function inventory(scene: SceneValue, layout: Layout): InventoryItem[] {
  const groups = new Map<string, InventoryItem>();
  for (const node of layout.nodes) {
    const building = scene.payload.buildings.find(entry => entry.id === node.templateId);
  ...
  const occupied = buildingCells(layout, buildingIndex(scene));
  for (const [kind, item] of Object.entries(scene.lineItems)) {
...
export function metrics(scene: SceneValue, layout: Layout): PresentationMetrics {
  const context = document.createElement('canvas').getContext('2d')!;
  const details = detailsOf(layout);
  const nameLines = textLines(context, layout.name, 336, `500 34px "${FONT_FAMILY}",sans-serif`);
  const description = details.description
    ? textLines(context, details.description, 532, `24px "${FONT_FAMILY}",sans-serif`)
    : [];
  ...
  const creatorY = y - 35;
  context.font = `25px "${FONT_FAMILY}",sans-serif`;
  ...
  const items = inventory(scene, layout);
 ...
 */
export function viewportTransform(layout: Layout, m: PresentationMetrics, scene: SceneValue): ViewportTransform {
  const b = bounds(layout, buildingIndex(scene), 0);
...
function stamp(
  ...
  const image = scene.assets.get(assetKey);
  if (!image) return;
  c.save();
  c.globalAlpha *= alpha;
  if (contain) {
...
function label(
  ...
  c.font = `${weight ? `${weight} ` : ''}${size}px "${FONT_FAMILY}",sans-serif`;
  c.fillStyle = color;
  c.textBaseline = 'top';
  c.textAlign = 'left';
  c.fillText(text, x, y);
...
function drawCard(
  ...
  c.clip();
  stamp(c, scene, native.sprites.bg_item_small_black, x - 2, y - 2, w + 4, h + 4);
  stamp(c, scene, native.rarityLayers[item.rarityColor]!, x, y + 76, w, 54);
  c.fillStyle = item.rarityColor || '#999';
  c.fillRect(x, y + h - 6, w, 6);
  stamp(c, scene, native.covers[item.id]?.asset || item.palette, x + 2, y + 1, 126, 119, 1, true);
  stamp(c, scene, native.sprites.deco_item_dot, x + 5, y + 5, 36, 5, 0.55);
  if (item.count) {
    c.shadowColor = '#000';
    c.shadowBlur = 4;
    c.font = `32px "${FONT_FAMILY}",sans-serif`;
    c.fillStyle = '#f4f4f4';
    c.textAlign = 'center';
...
export function paintPresentation(
  ...
  const native = scene.presentation;
  ...
  stamp(c, scene, native.sprites.deco_fac_blueprint_6!, 0, 0, 970, 145, 0.6);
  stamp(c, scene, native.sprites.deco_fac_blueprint_5!, 1350, 0, 1120, 124, 0.6);
  stamp(c, scene, native.sprites.deco_fac_blueprint_24!, 65, 2, 216, 108, 0.22);
  stamp(c, scene, native.sprites.deco_assembly07_new!, 67, 28, 82, 15);
  stamp(c, scene, native.sprites.deco_fac_blueprint_28!, 1914, 34, 356, 60);
  label(c, '// 蓝图预览', 67, 48, 28, '#161616', '600');
  stamp(c, scene, native.sprites.close_btn_bg_shadeless!, 2430, 26, 64, 64);

  const t = viewportTransform(layout, m, scene);
  c.save();
  c.beginPath();
  ...
  stamp(c, scene, native.sprites.deco_fac_blueprint_light!, PANEL_X, 116, PANEL_W, H - 116);
  c.save();
  c.beginPath();
  c.rect(PANEL_X, 116, PANEL_W, 368);
  c.clip();
  for (let x = PANEL_X; x < PANEL_X + PANEL_W; x += 28)
    stamp(c, scene, native.sprites.deco_fac_blueprint_30!, x, 116, 28, 368);
  c.restore();
  c.save();
  c.beginPath();
  ...
  c.stroke();
  stamp(c, scene, native.sprites.icon_fac_blueprint_save_big!, 2290, 116, 226, 226);
  c.restore();
  let stripY = 116;
  ...
  stamp(c, scene, native.sprites[`icon_fac_blueprint_bg_${p.coverColor}`]!, 1908, 125, 240, 240, 1, true);
  ...
  stamp(c, scene, cover.asset, 2033 - iconSize / 2, 245 - iconSize / 2, iconSize, iconSize, 1, true);
  m.nameLines.forEach((line, i) => label(c, line, 2122, 224 + i * 43, 34, '#f5f5f3', '500'));
  c.font = `500 34px "${FONT_FAMILY}",sans-serif`;
  ...
  stamp(c, scene, native.sprites.deco_fac_blueprint_size!, 2110, sizeY + 1, 25, 25, 0.5);
  ...
  stamp(c, scene, native.sprites.bg_fac_blueprint_more!, 2446, m.creatorY - 10, 45, 45);
  ...
  m.items.forEach((item, i) =>
    drawCard(c, scene, native, item, 1940 + (i % 4) * 142, m.listY + Math.floor(i / 4) * 143),
  );
  if (!m.items.length) label(c, '放置设备后自动生成', 1940, m.listY + 8, 24, '#777');
  c.fillStyle = '#27282a';
  c.fillRect(2516, 116, 44, H - 116);
  for (let y = 195; y < H; y += 425) stamp(c, scene, native.sprites.deco_fac_blueprint_29!, 2527, y, 24, 425);
  stamp(c, scene, native.sprites.deco_fac_blueprint_25!, 0, 0, 2560, H, 0.5);
}
 ...
 */
export async function exportPresentation(scene: SceneValue, layout: Layout, width: number): Promise<HTMLCanvasElement> {
  const source = validate(clone(layout), buildingIndex(scene));
  if (!PREVIEW_WIDTHS.has(width)) throw Error('请选择有效的完整预览分辨率');

  const m = metrics(scene, source);
  const buildings = buildingIndex(scene);
  // Preload every asset the sheet can touch, including covers and rarity bars.
  const keys = new Set<string>(Object.values(scene.presentation.sprites));
  for (const item of m.items) {
    keys.add(scene.presentation.covers[item.id]?.asset ?? item.palette);
    keys.add(scene.presentation.rarityLayers[item.rarityColor] ?? '');
  }
  keys.add(scene.presentation.covers[m.details.coverId]?.asset ?? scene.presentation.covers['']!.asset);
  await scene.assets.preload([...keys].filter(Boolean));
  await scene.assets.preload(
    source.nodes.flatMap(node => {
      ...
      const product: Product | undefined = node.productIcon
        ? scene.products[node.productIcon.startsWith('[gas]') ? node.productIcon.toLowerCase() : node.productIcon]
        : undefined;
      return [
  ...
  paintPresentation(c, scene, source, m);
  return out;
}

/** All underground connections available for the "show connection" annotation. */
export function connectionOptions(layout: Layout): Array<{ value: string; text: string }> {
